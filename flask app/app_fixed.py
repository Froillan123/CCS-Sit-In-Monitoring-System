...
def sit_in_reservation(reservation_id):
    try:
        print(f"Attempting to approve reservation with ID: {reservation_id}")
        ...
        if update_reservation_status(reservation_id, 'Approved'):
            print(f"Successfully updated reservation {reservation_id} status to 'Approved'")
            
            # Insert session history record for all reservation types
            login_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            insert_session_history(reservation['student_idno'], login_time, None)
        ...
        print(f"Error in sit-in reservation for {reservation_id}: {str(e)}")
...
def logout_student(reservation_id):
    ...
    try:
        # Get reservation information with all the needed fields
        query = """
            SELECT 
                r.*, 
                s.sessions_left, 
            ...
            WHERE 
                r.id = ?
        """
        result = getprocess(query, (reservation_id,))
        ...
        # Only active reservations can be logged out
        if current_status != 'Approved':
            return jsonify({"success": False, "message": f"Cannot logout a reservation with status '{current_status}'"}), 400
        
        # Set logout time
        logout_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        # Update session history with logout time for all reservation types
        update_session_history(student_idno, logout_time)
        
        # For reservations, set the computer to available without updating status to "Logged Out"
        if reservation['reservation_type'] == 'reservation':
            # Update reservation logout time without changing status
            postprocess(
                "UPDATE reservations SET logout_time = ? WHERE id = ?",
                (logout_time, reservation_id)
            )
            
            # Free up the computer if one was assigned
            if reservation.get('computer_id'):
                update_computer_status(reservation['computer_id'], 'Available')
                
            # Decrement the student's sessions for reservations too
            ...
            add_reservation_notification(
                ...
                f"Your reservation in {reservation['lab_name']} has been completed.",
                "status_update"
            )
            ...
            socketio.emit('reservation_updated', {
                ...
                'computer_number': reservation.get('computer_number')
            })
        ...
        # For sit-ins, handle normal logout process
        update_query = """
            UPDATE reservations 
            SET status = 'Logged Out', logout_time = ? 
            WHERE id = ?
        """
        
        if not postprocess(update_query, (logout_time, reservation_id)):
        ...
        add_reservation_notification(
            ...
            reservation_id,
            "Sit-In Logged Out",
            f"Your sit-in in {reservation['lab_name']} has been logged out by admin.",
            "status_update"
        )
        ...
        app.logger.error(f"Error in logout_student: {str(e)}")
...
def admin_create_sitin():
    ...
    sql = """
        ...
        (student_idno, student_name, lab_id, purpose, reservation_date, 
         login_time, status, session_number, reservation_type) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    params = (
        student_id, 
        ...
        'Approved',
        session_number,
        'sit-in'  # Set reservation type to 'sit-in'
    )
    ...
    insert_session_history(student_id, login_time, None)
    ...
    add_reservation_notification(
        ...
        new_reservation['id'],
        "New Sit-In Created",
        f"An administrator has created a sit-in session for you in {new_reservation['lab_name']}.",
        "status_update"
    )

    # Get the updated student information with remaining sessions
    updated_student = get_student_by_idno(student_id)
    ...
    return jsonify({
        ...
        "reservation": {
            ...
            "lab_name": new_reservation['lab_name'],
            "purpose": purpose,
            "login_time": login_time,
            "status": 'Approved',
            "sessions_left": updated_student['sessions_left']
        }
    })
    ...
    try:
        query = """
            SELECT 
                ...
                lc.computer_number,
                CASE 
                    WHEN r.login_time IS NOT NULL AND r.logout_time IS NOT NULL 
                    THEN time(r.login_time) || ' - ' || time(r.logout_time) 
                    WHEN r.login_time IS NOT NULL 
                    THEN time(r.login_time) || ' - Present' 
                    ELSE r.time_slot 
                END AS display_time_slot
            FROM reservations r
...
def get_currentsitin():
    """Get all currently active sit-ins and approved reservations (excluding logged out)"""
    try:
        # Check if admin is logged in
        if 'admin_username' not in session:
            return jsonify({"success": False, "message": "Unauthorized"}), 401
            
        # Get current date and time for filtering reservations
        current_date = datetime.now().strftime('%Y-%m-%d')
        current_time = datetime.now().strftime('%H:%M:%S')
            
        # Get all current sit-ins and approved reservations (not logged out and without logout time)
        query = """
            SELECT 
                r.id, r.student_idno, r.student_name, l.lab_name, r.purpose, 
                r.reservation_date, r.login_time, r.logout_time, r.status,
                r.session_number, r.time_slot,
                r.reservation_type,
                lc.computer_number
            FROM reservations r
            JOIN laboratories l ON r.lab_id = l.id
            LEFT JOIN lab_computers lc ON r.computer_id = lc.id
            WHERE r.status != 'Logged Out' 
            AND r.status != 'Rejected'
            AND r.logout_time IS NULL
            AND r.reservation_date = ?
            AND (
                /* Show all sit-ins for today */
                (r.reservation_type = 'sit-in' AND r.status = 'Approved')
                OR 
                /* Show reservations only if current time is within their time slot */
                (r.reservation_type = 'reservation' AND r.status = 'Approved' AND r.time_slot LIKE '%-%' 
                 AND time(?) BETWEEN time(substr(r.time_slot, 1, 5)) AND time(substr(r.time_slot, -5)))
            )
            ORDER BY 
        ...
        reservations = getprocess(query, (current_date, current_time))
        
        return jsonify({"success": True, "data": reservations})
    except Exception as e:
        print(f"Error in get_currentsitin: {e}")
        return jsonify({"success": False, "message": f"Failed to fetch current sit-ins: {e}"}), 500
...
def award_points():
    ...
    try:
        ...
        result = add_points_to_student(
            student_idno=student_idno,
            points=1,
            reason=f"Reward for {reservation_type} {reservation_id}" if reservation_id else reason,
        ...
        message = f"Successfully awarded 1 point for {reservation_type}."
...
def api_add_lab_schedule():
    ...
    end_time = data.get('end_time')  # This is now optional
    status = data.get('status')
    ...
    # Validate required fields - end_time is no longer required
    if not all([lab_id, day_of_week, start_time, status]):
        return jsonify({"success": False, "message": "Missing required fields"}), 400
...
def update_student_reservation():
    ...
    try:
        ...
        reservation = getprocess(
            "SELECT * FROM reservations WHERE id = ? AND student_idno = ?", 
        ...
        if purpose:
            success = postprocess(
                "UPDATE reservations SET purpose = ? WHERE id = ?",
...
def get_pending_reservations():
    ...
    try:
        query = """
            SELECT r.id, r.student_idno, r.student_name,
                   l.lab_name, lc.computer_number, r.reservation_date, r.time_slot, 
                   r.purpose, r.created_at, r.status, r.reservation_type
            FROM reservations r
            JOIN laboratories l ON r.lab_id = l.id
            LEFT JOIN lab_computers lc ON r.computer_id = lc.id
            WHERE r.status = 'Pending'
...
def update_reservation_status_endpoint():
    ...
    try:
        ...
        # Get current reservation info before update
        query = """
        SELECT r.*, s.sessions_left, l.lab_name
        FROM reservations r
        JOIN students s ON r.student_idno = s.idno
        JOIN laboratories l ON r.lab_id = l.id
        WHERE r.id = ?
        ...
        # For reservations, only allow Approved or Rejected status
        if reservation['reservation_type'] == 'reservation':
            if status not in ['Approved', 'Rejected']:
                return jsonify({'success': False, 'message': 'Invalid status. For reservations, only Approved or Rejected are allowed'}), 400
        else:
            # For sit-ins, allow all statuses
            if status not in ['Approved', 'Rejected', 'Logged Out']:
                return jsonify({'success': False, 'message': 'Invalid status. Must be Approved, Rejected, or Logged Out'}), 400
        
        # Don't allow status changes from Logged Out
        if current_status == 'Logged Out':
            return jsonify({'success': False, 'message': 'Cannot change status of logged out reservations'}), 400
            
        # If approving a sit-in reservation, check if student has sessions left
        if status == 'Approved' and reservation['reservation_type'] == 'sit-in' and reservation['sessions_left'] <= 0:
            return jsonify({'success': False, 'message': 'Student has no sessions left for sit-in'}), 400
        ...
        update_query = """
            ...
            WHERE id = ?
        """
        if not postprocess(update_query, (status, reservation_id)):
            return jsonify({'success': False, 'message': 'Failed to update reservation status'}), 500
        
        # Add login_time and create session history when approving a reservation
        if status == 'Approved' and current_status != 'Approved':
            login_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            # For sit-ins and reservations, create session history
            insert_session_history(student_idno, login_time, None)
        ...
        if status == 'Logged Out':
            ...
            # Update reservation's logout time
            postprocess(
                "UPDATE reservations SET logout_time = ? WHERE id = ?",
                (logout_time, reservation_id)
            )
            ...
            # Decrement sessions if it's a sit-in
            if reservation['reservation_type'] == 'sit-in' and reservation['sessions_left'] > 0:
                new_sessions = reservation['sessions_left'] - 1
                update_student_sessions(student_idno, new_sessions)
        
        # Create a notification for the student
        notification_title = f"Reservation {status}"
        notification_message = f"Your {reservation['reservation_type']} for {reservation['lab_name']} on {reservation['reservation_date']} has been {status.lower()}."
        ...
        # Emit student-specific socket event
        socketio.emit(f'student_{student_idno}_reservation_updated', {
            'reservation_id': reservation_id,
            'status': status
        })
        ...
        app.logger.error(f"Error updating reservation status: {str(e)}")
...
def get_processed_reservations():
    ...
    try:
        ...
        if status != 'all':
            query_parts.append("AND r.status = ?")
            params.append(status)
        
        # Add date filter if provided
        if date:
            query_parts.append("AND r.reservation_date = ?")
            params.append(date)
        ...
        # Execute the query
        reservations = getprocess(query, tuple(params))
        ...
        app.logger.error(f"Error fetching processed reservations: {str(e)}")
        return jsonify({"success": False, "message": f"Error: {str(e)}"}), 500
...
def get_student_notifications_route():
    ...
    try:
        ...
        reservation_type = request.args.get('reservation_type')  # Can be 'reservation' or 'sit-in'
        
        # Get notifications from the database
        notifications = get_student_notifications(student_idno, limit, include_read, reservation_type)
        ...
        app.logger.error(f"Error fetching student notifications: {str(e)}")
...
def mark_notification_read_route():
    ...
    try:
        ...
        # Verify this notification belongs to the student
        student_idno = session['student_idno']
        check_query = """
            SELECT id FROM reservation_notifications
            WHERE id = ? AND student_idno = ?
        """
        notification = getprocess(check_query, (notification_id, student_idno))
        ...
        success = postprocess(update_query, (notification_id,))
...
def mark_all_notifications_read_route():
    ...
    try:
        ...
        success = postprocess(update_query, (student_idno,))
...
def get_unread_notifications_count_route():
    ...
    try:
        ...
        result = getprocess(query, (student_idno,))
        ...
        app.logger.error(f"Error getting unread notifications count: {str(e)}")
        return jsonify({"success": False, "message": f"Error: {str(e)}"}), 500

# Add a scheduled task to check for upcoming reservations and send notifications
def check_upcoming_reservations():
    """Check for upcoming reservations and notify students"""
    try:
        # Get reservations starting in the next 30 minutes
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                SELECT r.*, lc.computer_number, s.email
            FROM reservations r
            LEFT JOIN lab_computers lc ON r.computer_id = lc.id
                LEFT JOIN students s ON r.student_idno = s.student_idno
                WHERE r.status = 'confirmed'
                AND r.start_time BETWEEN %s AND %s
                AND r.notification_sent = FALSE
            """, (current_time, thirty_mins_from_now))
        ...
        for reservation in upcoming_reservations:
                ...
                notification = {
                    ...
                    'message': f"Your reservation starts in 30 minutes. Computer #{reservation['computer_number']} has been assigned to you.",
                    'type': 'upcoming',
                    'reservation_id': reservation['id']
                }
                
                # Send notification
                socketio.emit('student_notification', notification)
                ...
                if reservation['email']:
                    send_email(
                        ...
                        f"Your reservation starts in 30 minutes. Computer #{reservation['computer_number']} has been assigned to you."
                    )
            ...
            cursor.execute("""
                ...
                WHERE s.is_waiting_for_sit_in = TRUE
                AND s.current_computer IS NULL
                GROUP BY s.student_idno
            ...
            for computer in available_computers:
                if waiting_students:
                    ...
                    notification = {
                        ...
                        'title': 'Sit-in Opportunity Available',
                        'message': f"Computer #{computer['computer_number']} is now available for sit-in. Please proceed to the computer lab.",
                        'type': 'sit_in',
                        'computer_id': computer['id']
                    }
                    ...
                    cursor.execute("""
                        UPDATE students 
                        SET is_waiting_for_sit_in = FALSE,
                            current_computer = %s,
                            waiting_since = NULL
                        WHERE student_idno = %s
                    """, (computer['id'], student['student_idno']))
                    
                    # Update computer status
                    cursor.execute("""
                        UPDATE lab_computers 
                        SET status = 'Occupied',
                            is_available_for_sit_in = FALSE
                        WHERE id = %s
                    """, (computer['id'],))
...
# Schedule the check_upcoming_reservations function to run every minute
@socketio.on('connect')
def handle_connect():
    if not hasattr(handle_connect, 'scheduler_started'):
        from apscheduler.schedulers.background import BackgroundScheduler
        scheduler = BackgroundScheduler()
        scheduler.add_job(check_upcoming_reservations, 'interval', minutes=1)
        scheduler.start()
        handle_connect.scheduler_started = True

def check_reservation_time_slots():
    """Check for reservations that have started or ended based on time slots"""
    try:
        # Get the current date and time
        current_date = datetime.now().strftime("%Y-%m-%d")
        current_time = datetime.now().strftime("%H:%M:%S")
        
        # 1. Find reservations whose time slots have started - ONLY send notification
        start_query = """
            SELECT r.*, s.idno as student_idno, l.lab_name, lc.computer_number
            FROM reservations r
            JOIN students s ON r.student_idno = s.idno
            JOIN laboratories l ON r.lab_id = l.id
            LEFT JOIN lab_computers lc ON r.computer_id = lc.id
            ...
            AND r.time_slot LIKE '%-%'
            AND time(substr(r.time_slot, 1, 5)) <= time(?)
            AND NOT EXISTS (
                SELECT 1 FROM reservation_notifications rn 
                WHERE rn.reservation_id = r.id AND rn.notification_type = 'start_time'
            )
        """
        
        started_reservations = getprocess(start_query, (current_date, current_time))
        
        for reservation in started_reservations:
            title = "Reservation Started"
            message = f"Your reservation time slot for {reservation['lab_name']} has started. You can now proceed to the lab."
        ...
        # 2. Find reservations whose time slots have ended - ONLY send notification
        end_query = """
            SELECT r.*, s.idno as student_idno, l.lab_name, lc.computer_number
            FROM reservations r
            JOIN students s ON r.student_idno = s.idno
            ...
            AND r.status = 'Approved'
            AND r.time_slot LIKE '%-%'
            AND time(substr(r.time_slot, -5)) <= time(?)
            AND NOT EXISTS (
                SELECT 1 FROM reservation_notifications rn 
                WHERE rn.reservation_id = r.id AND rn.notification_type = 'end_time'
            )
        """
        ...
        for reservation in ended_reservations:
            # Only create a notification, don't change any status
            title = "Reservation Time Ended"
            message = f"Your reservation time slot for {reservation['lab_name']} has ended. The administrator will manage your session."
            ...
            # Emit socket event to notify the student
            socketio.emit('reservation_time_ended', {
                'student_idno': reservation['student_idno'],
                'reservation_id': reservation['id'],
                'title': title,
                'message': message,
            ...
            socketio.emit('admin_reservation_time_ended', {
                'student_idno': reservation['student_idno'],
                'reservation_id': reservation['id'],
                'lab_name': reservation['lab_name'],
                'computer_number': reservation['computer_number'],
                'message': f"Reservation for {reservation['student_idno']} in {reservation['lab_name']} (PC #{reservation['computer_number']}) has ended",
                'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            })
...
@app.route('/get_logged_out_reservations')
def get_logged_out_reservations():
    try:
        student_idno = session.get('idno')
        if not student_idno:
            return jsonify({"success": False, "message": "Not logged in"}), 401
            
        reservations = get_student_logged_out_reservations(student_idno)
        ...
        print(f"Error getting logged out reservations: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def create_reservation_route():
    ...
    if 'student_idno' not in session:
        return jsonify({"success": False, "message": "You must be logged in to create a reservation"}), 401
    ...
    try:
        ...
        reservation_type = data.get('reservation_type', 'reservation')  # Default to reservation
        
        # Validate required fields
        if not all([lab_id, purpose, reservation_date]):
            return jsonify({"success": False, "message": "Missing required fields"}), 400
        
        # Check if time slot is required for this type
        if reservation_type == 'reservation' and not time_slot:
            return jsonify({"success": False, "message": "Time slot is required for reservations"}), 400
        
        # Prevent invalid reservation dates (past dates)
        try:
            reservation_date_obj = datetime.strptime(reservation_date, '%Y-%m-%d')
            today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            if reservation_date_obj < today:
                return jsonify({"success": False, "message": "Cannot create reservations for past dates"}), 400
        except ValueError:
            return jsonify({"success": False, "message": "Invalid date format"}), 400
        
        # Check if the student has sessions left for sit-in
        if reservation_type == 'sit-in':
            student = get_student_by_idno(student_idno)
            if not student or student['sessions_left'] <= 0:
                return jsonify({
                    "success": False, 
                    "message": "You don't have any sessions left for sit-ins. Consider making a reservation instead."
                }), 400
        
        # Check for pending reservations - a student can have only one pending reservation at a time
        pending_check_query = """
            SELECT COUNT(*) as count 
            FROM reservations 
            WHERE student_idno = ? AND status = 'Pending'
        """
        result = getprocess(pending_check_query, (student_idno,))
        if result and result[0]['count'] > 0:
            return jsonify({
                "success": False, 
                "message": "You already have a pending reservation. Please wait for approval."
            }), 400
        ...
        # Get lab name for notification
        lab_query = "SELECT lab_name FROM laboratories WHERE id = ?"
        lab_result = getprocess(lab_query, (lab_id,))
        lab_name = lab_result[0]['lab_name'] if lab_result else "Unknown lab"
